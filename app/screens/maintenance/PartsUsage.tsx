...
import { fetchVanStock, type VanStockItem } from '@/lib/inventoryService';
...
export default function PartsUsageScreen() {
  ...
  const loadVanStock = useCallback(async () => {
    try {
      setLoading(true);
      setLoadError(null);
      const rows = await fetchVanStock(undefined, { search: search || undefined });
      setVanStock(rows.filter((item) => item.location === 'Van Stock'));
    } catch (error) {
      const message =
        error instanceof ApiError ? error.message : 'Could not load van stock';
  ...
  const handleConfirm = () => {
    const selectedParts = Object.entries(selections).map(([itemId, qty]) => {
      const item = vanStock.find((i) => i.id === itemId);
      return {
        id: itemId,
    ...
    <View style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        <View style={styles.searchBar}>
          ...
          <TextInput
            ...
            onSubmitEditing={() => void loadVanStock()}
      ...
      {loading ? (
        ...
        <View style={styles.centered}>
          ...
          <Button title="Retry" variant="outline" onPress={() => void loadVanStock()} />
        </View>
      ) : (
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.list}>
            {vanStock.length === 0 ? (
              <Text style={styles.emptyText}>No van stock items found.</Text>
            ) : (
              vanStock.map((item, i) => {
                    ...
                    <Card style={[styles.partCard, qty > 0 ? styles.partCardSelected : null]}>
                      <View style={styles.partMain}>
                        ...
                        <View style={styles.qtyControl}>
                          <TouchableOpacity
                            onPress={() => updateQty(item.id, -1, item.quantity)}
                            style={[styles.qtyBtn, qty === 0 && styles.qtyBtnDisabled]}
                            disabled={qty === 0}
                          >
                            <Minus
                              size={16}
                              color={qty === 0 ? colors.border : colors.foreground}
                            />
                          </TouchableOpacity>

                          <View style={styles.qtyValueBox}>
                            <Text style={[styles.qtyText, qty > 0 && styles.qtyTextActive]}>
                              {qty}
                            </Text>
                          </View>
                          ...
                          >
                            <Plus
                              size={16}
                              color={qty >= item.quantity ? colors.border : colors.foreground}
                            />
                          </TouchableOpacity>
                        </View>
                      </View>
...
const getStyles = (colors: any) =>
  StyleSheet.create({
    ...
    scrollableFooter: { marginTop: 32, backgroundColor: 'transparent' },
    ...
    errorText: { color: colors.destructive, textAlign: 'center', marginBottom: 8 },
    emptyText: { color: colors.mutedForeground, textAlign: 'center', padding: 24 },
  });
